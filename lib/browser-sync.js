...
/**
 * In proxy mode, add the middleware to the 'proxyRes' config
 * option to ensure these headers are set AFTER the server
 * has sent it's response
 * In server mode, just add the middleware
 */
BrowserSync.prototype.addDisableBrowserCacheMiddleware = function () {
    var bs = this;
    var headers = bs.options.get("disableBrowserCacheHeaders").toJS();
    ...
    if (bs.options.get("mode") === "proxy") {
        ...
        fn.id = "bs-disable-cache";
        bs.proxy.config.proxyRes.push(fn);
    } else {
        bs.addMiddleware("*", function (req, res, next) {
            ...
            next();
        }, {id: "bs-disable-cache", override: true});
    }
};

/**
 * Remove the disableBrowserCache middleware
 */
BrowserSync.prototype.removeDisableBrowserCacheMiddleware = function () {
    var bs = this;

    if (bs.options.get("mode") === "proxy") {
        bs.proxy.config.proxyRes = bs.proxy.config.proxyRes.filter(function (item) {
            return item.id !== "bs-disable-cache";
        });
    } else {
        bs.removeMiddleware("bs-disable-cache");
    }
};
