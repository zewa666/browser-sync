...
module.exports = {
    ...
    /**
     * Add middleware to server/proxy
     * to force the browser to re-fetch assets.
     * @param bs
     * @param done
     * @returns {*}
     */
    addCacheDisableMiddleware: function (bs, done) {
        if (!bs.options.get("disableBrowserCache")) {
            return done();
        }
        bs.addDisableBrowserCacheMiddleware();
        done();
    },
    /**
     * @param {BrowserSync} bs
